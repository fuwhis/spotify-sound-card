...
/** Full-page <a> — Next.js <Link> soft-fetches API redirects and trips CORS on Spotify. */
  ...
  if (params.error) {
    ...
    try {
      ...
      if (!data.refresh_token) {
        setupResult = {
          error: 'No refresh token returned. Try authorizing again.',
        }
      } else {
  ...
  return (
    <div className='min-h-full bg-[#0a0a0a] text-zinc-100'>
            ...
            {'refreshToken' in setupResult ? (
              <>
                ...
                <p className='mt-2 text-xs text-zinc-500'>
                  Also add to <code className='text-zinc-400'>.env.local</code>{' '}
                  for local dev.
                </p>
              </>
            ) : (
              <>
                ...
                <AuthorizeLink className='mt-4 inline-block text-sm text-[#1DB954] hover:underline'>
                  Try again →
                </AuthorizeLink>
              </>
            )}
      ...
      <section className='mx-auto max-w-2xl px-6 pb-10'>
        ...
        <AuthorizeLink className='inline-flex items-center rounded-full bg-[#1DB954] px-5 py-2.5 text-sm font-medium text-black transition hover:bg-[#1ed760]'>
          Authorize Spotify →
        </AuthorizeLink>
      </section>
      ...
      <section className='mx-auto max-w-2xl px-6 pb-10'>
        ...
        <p className='mt-2 text-xs text-zinc-600'>
          Edge soft-caches ~30s; GitHub Camo revalidates immediately
          (max-age=0). Play something on Spotify to see the waveform.
        </p>
      </section>
      ...
      <section className='mx-auto max-w-2xl px-6 pb-20'>
        ...
        <ol className='list-decimal list-inside space-y-2 text-sm text-zinc-400 leading-relaxed'>
          ...
          <li>
            ...
            <code className='text-zinc-300'>SPOTIFY_CLIENT_SECRET</code> on
            Vercel
          </li>
          <li>
            Open{' '}
            <AuthorizeLink className='text-[#1DB954] hover:underline'>
              /api/spotify/login
            </AuthorizeLink>{' '}
            (or click Authorize above) to get refresh token
          </li>
          <li>
            Add <code className='text-zinc-300'>SPOTIFY_REFRESH_TOKEN</code> on
            Vercel and redeploy
          </li>
        </ol>
      </section>
